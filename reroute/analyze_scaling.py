...
def analyze_scaling():
    ...
    configurations = [
        ...
        {"name": "City-Scale", "grid": (60, 60), "stops": 400, "buses": 150},
    ]
    ...
    if successful_configs:
        ...
        for config in successful_configs:
            if config['success']:
                print(f"  {config['name']}: {config['step_time']:.3f}s per step, {config['memory_used']:.1f} MB")
        
        # Recommendations
        print(f"\n💡 RECOMMENDATIONS:")
        print(f"  ✅ Current system can handle up to {largest['stops']} stops")
        print(f"  ✅ Memory usage is reasonable ({largest['memory_used']:.1f} MB)")
        ...
        print(f"  Expected memory: ~50-100 MB")
