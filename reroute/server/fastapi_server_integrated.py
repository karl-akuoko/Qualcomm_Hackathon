...
async def root():
    ...
    <head>
        ...
        <style>
            ...
            .info-panel { background: #f8fafc; padding: 15px; border-radius: 8px; margin: 10px 0; }
            .train-btn { background: #8b5cf6; color: white; }
        </style>
    </head>
    <body>
        <div class="container">
            <div class="header">
                ...
                <div>Time: <span id="time">0.0</span>m | Mode: <span id="mode">static</span> | RL Available: <span id="rl-available">No</span></div>
            </div>
            ...
            <div class="controls">
                ...
                <button class="btn train-btn" onclick="startTraining()">Train RL Model</button>
        ...
        <script>
            ...
            fetch('/status')
                ...
                .then(data => {
                    ...
                    document.getElementById('rl-available').textContent = data.rl_available ? 'Yes' : 'No';
                    document.getElementById('system-info').innerHTML = `
                        <strong>Buses:</strong> ${data.buses || 0} | 
                        <strong>Stops:</strong> ${data.stops || 0} | 
                        <strong>Connected Clients:</strong> ${data.connected_clients || 0}
                    `;
                })
...
async def startup_event():
    ...
    if os.path.exists(onnx_path) and ONNX_AVAILABLE:
        ...
        print("Run training first to enable RL mode")
...
async def start_training(request: dict):
    ...
    try:
        timesteps = request.get('timesteps', 10000)
        
        # Run training in background
        asyncio.create_task(run_training(timesteps))
...
async def run_training(timesteps: int):
    """Run training in background"""
    
    print(f"Starting training with {timesteps} timesteps...")
    ...
    try:
        ...
        result = subprocess.run([
            sys.executable, 'train_working.py'
        ], capture_output=True, text=True)
        
        if result.returncode == 0:
            print("✓ Training completed successfully")
            # Try to reload ONNX policy
            await reload_onnx_policy()
        else:
...
async def reload_onnx_policy():
    """Reload ONNX policy after training"""
    global onnx_policy
    ...
    if os.path.exists(onnx_path) and ONNX_AVAILABLE:
        try:
            onnx_policy = ONNXPolicyInference(onnx_path)
            print("✓ ONNX policy reloaded successfully")
        except Exception as e:
            print(f"✗ Failed to reload ONNX policy: {e}")
