...
app = FastAPI(title="Bus Dispatch RL - Final Fix", version="1.0.0")
...
async def root():
    """Serve the dashboard with final fixes"""
    dashboard_html = """
    <!DOCTYPE html>
    <html lang="en">
    <head>
        ...
        <title>Bus Dispatch RL - Final Fix</title>
        <style>
    ...
    <body>
        <div class="container mobile-optimized">
            <div class="header">
                <h1>🚌 Bus Dispatch RL - Final Fix</h1>
            ...
            <div class="card">
                <h3>Manhattan Grid Simulation - Final Fix</h3>
...
async def startup_event():
    """Initialize simulation environment with final fixes"""
    global simulation_env
    
    print("Initializing Bus Dispatch System with Final Fixes...")
    ...
    print("Bus Dispatch System with Final Fixes initialized successfully!")

async def simulation_loop():
    """Simulation loop with final fixes"""
    ...
    while is_running:
        try:
            ...
            # Update simulation data with proper data structure handling
            simulation_data = get_fixed_system_state()
...
def get_fixed_system_state():
    """Get system state with proper data structure handling"""
    
    if simulation_env is None:
        return {"time": 0.0, "buses": [], "stops": [], "kpi": {"avg_wait": 0.0, "load_std": 0.0}}
    
    # Get bus states - handle the fact that buses might be stored differently
    buses = []
    try:
        ...
        print(f"Error getting bus states: {e}")
        buses = []
    
    # Get stop states - handle the fact that stops are stored as a dictionary
    stops = []
    total_waiting = 0
    try:
        for stop_id, stop in simulation_env.city_grid.stops.items():
        ...
        print(f"Error getting stop states: {e}")
        stops = []
...
async def shutdown_event():
    ...
    global is_running
    is_running = False
    print("Shutting down Bus Dispatch System with Final Fixes...")
