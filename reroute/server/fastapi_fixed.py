...
app = FastAPI(title="Bus Dispatch RL Fixed API", version="1.0.0")
...
async def root():
    """Serve the fixed dashboard with proper passenger boarding"""
    dashboard_html = """
    <!DOCTYPE html>
    <html lang="en">
    <head>
        ...
        <title>Bus Dispatch RL - Fixed</title>
        <style>
        ...
        <script>
            ...
            function createGrid() {
                ...
                for (let i = 0; i <= gridSize; i++) {
                    ...
                    line.style.top = (i * (400 / gridSize)) + 'px';
                ...
                for (let i = 0; i <= gridSize; i++) {
                    ...
                    line.style.left = (i * (400 / gridSize)) + 'px';
                    gridLines.appendChild(line);
                }
            }
            ...
            function updateMap() {
                ...
                if (systemState.stops) {
                    systemState.stops.forEach(stop => {
                        ...
                        stopElement.style.left = (stop.x * (400 / gridSize) - 8) + 'px';
                        stopElement.style.top = (stop.y * (400 / gridSize) - 8) + 'px';
                ...
                if (systemState.buses) {
                    systemState.buses.forEach(bus => {
                        ...
                        busElement.style.left = (bus.x * (400 / gridSize) - 10) + 'px';
                        busElement.style.top = (bus.y * (400 / gridSize) - 10) + 'px';
...
async def startup_event():
    """Initialize simulation environment with proper passenger boarding"""
    global simulation_env
    
    print("Initializing Fixed Bus Dispatch System...")
    
    # Initialize environment with proper settings for passenger boarding
    simulation_env = BusDispatchEnv(
        grid_size=(20, 20),
        num_stops=32,
        num_buses=6,
        time_step=2.0,  # 2 minute steps for better passenger boarding
    ...
    print("Fixed system initialized successfully!")

async def simulation_loop():
    """Fixed simulation loop with proper passenger boarding"""
    ...
    while is_running:
        try:
            ...
            # Print debug info
            if simulation_data.get('buses'):
                for bus in simulation_data['buses']:
                    if bus['load'] > 0:
                        print(f"Bus {bus['id']} has {bus['load']} passengers!")
            
            # Broadcast state to WebSocket clients
            await broadcast_state()
            
            # Control simulation speed (0.5 Hz for better observation)
...
async def shutdown_event():
    ...
    global is_running
    is_running = False
    print("Shutting down Fixed Bus Dispatch System...")
