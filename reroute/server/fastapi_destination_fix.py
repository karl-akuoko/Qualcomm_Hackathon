...
try:
    ...
    if os.path.exists("best_bus_model.zip"):
        ...
        print("⚠ No trained model found, using destination-aware mode")
except ImportError:
    print("⚠ Stable Baselines3 not available, using destination-aware mode")
    trained_model = None
...
simulation_env = None
current_mode = "destination_aware"
is_running = False
...
app = FastAPI(title="Bus Dispatch RL - Destination Fix", version="1.0.0")
...
async def root():
    """Serve the dashboard with destination-aware system"""
    dashboard_html = """
    <!DOCTYPE html>
    <html lang="en">
    <head>
        ...
        <title>Bus Dispatch RL - Destination Fix</title>
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .bus.destination_aware { background: #8b5cf6; }
            .bus.loaded { 
    ...
    <body>
        <div class="container mobile-optimized">
            <div class="header">
                <h1>🚌 Bus Dispatch RL - Destination Fix</h1>
                <div id="status" class="status disconnected">Disconnected</div>
                <div>Time: <span id="time">0.0</span>m | Mode: <span id="mode">destination_aware</span> | RL Model: <span id="rl-status">Not Available</span></div>
                <div style="margin-top: 10px; font-size: 12px; color: #6b7280;">
                    ✅ Destination-aware routing | ✅ Passenger drop-off fixed | ✅ Wait time = arrival to destination
                </div>
            </div>
            ...
            <div class="controls">
                ...
                <button class="btn btn-info" onclick="switchMode('destination_aware')">Destination-Aware</button>
            ...
            <div class="legend">
                ...
                <div class="legend-item">
                    ...
                    <span>Destination-Aware Buses</span>
                </div>
                <div class="legend-item">
            ...
            <div class="card">
                <h3>Manhattan Grid Simulation - Destination-Aware System</h3>
        ...
        <script>
            ...
            fetch('/status')
                ...
                .then(data => {
                    document.getElementById('mode').textContent = data.mode || 'destination_aware';
...
async def startup_event():
    """Initialize simulation environment with destination-aware system"""
    global simulation_env
    
    print("Initializing Bus Dispatch System with Destination-Aware Routing...")
    
    # Initialize environment with better balance
    ...
    print("Bus Dispatch System with Destination-Aware Routing initialized successfully!")
    print(f"✅ Destination-aware: {simulation_env.num_buses} buses for {simulation_env.num_stops} stops")
    print(f"✅ Ratio: {simulation_env.num_buses / simulation_env.num_stops:.2f} buses per stop")

async def simulation_loop():
    """Simulation loop with destination-aware routing"""
    ...
    while is_running:
        try:
            ...
            elif current_mode == "destination_aware":
                # Destination-aware routing
                actions = generate_destination_aware_actions()
            else:
...
def generate_destination_aware_actions():
    """Generate actions that route buses to passenger destinations"""
    actions = []
...
async def set_mode(request: ModeRequest):
    """Switch between static, RL, and destination-aware modes"""
    global current_mode
    
    if request.mode not in ["static", "rl", "destination_aware"]:
        return {"error": "Mode must be 'static', 'rl', or 'destination_aware'"}
    ...
    if current_mode == "static":
        ...
        simulation_env.bus_fleet.set_mode(BusMode.RL)  # Use RL mode for destination-aware
    else:
...
async def shutdown_event():
    ...
    global is_running
    is_running = False
    print("Shutting down Bus Dispatch System with Destination-Aware Routing...")
